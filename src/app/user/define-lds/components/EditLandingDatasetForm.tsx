...
interface EditLandingDatasetFormProps {
...
export function EditLandingDatasetForm({
  dataset,
  onInputChange,
  onSubmit,
  onTest,
  isTestSuccessful,
}: EditLandingDatasetFormProps) {
  ...
  return (
    <div className="bg-white shadow rounded-lg p-6">
      ...
      <form onSubmit={onSubmit} className="space-y-4 grid grid-cols-2 gap-4">
        ...
        <div>
          <label
            htmlFor="lnddsshortname"
            className="block text-sm font-medium text-gray-700"
          >
            Landing Datastore Name
          </label>
          <input
            type="text"
            id="lnddsshortname"
            name="lnddsshortname"
            value={dataset.lnddsshortname}
            onChange={onInputChange}
            className="mt-1 block w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500 sm:text-sm"
          />
        </div>
