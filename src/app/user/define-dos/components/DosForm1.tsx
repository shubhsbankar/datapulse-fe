...
export function DosForm1({
  ...
  const [componentType, setComponentType] = useState('dh');
  ...
  // Get component names from DH table
  const dhComponents = useAppSelector(state =>
    // state.userfeat.rdvcomdh
    state.userfeat.rdvcomdh.filter(dh => dh.comptype === 'dh')
  );
  ...
  // Get unique project names from dh
  ...
  const uniqueBkcAreas = Array.from(new Set(['default', ...tenantBkccs.filter(t => t.tenantid == tenantId).map(t => t.bkcarea)]));

  // console.log({dhComponents});
  // const uniqueComponentNames = Array.from(new Set(dhComponents.filter(dh => dh.projectshortname == selectedProject && dh.dsname == selectedDataset).map(dh => dh.compname)))

  // Get satellite names and versions from DS table
  const dsComponents = useAppSelector(state =>
    state.userfeat.rdvcompds.filter(ds => ds.assoccomptype === 'dh')
  );

  const uniqueComponentNames = Array.from(new Set(
    dhComponents.filter(dh => dh.projectshortname === selectedProject && dh.dpname === selectedDataProduct && dh.dsname === selectedDataset && dh.comptype === componentType).map(dh => dh.compname)
    // .map(ds => ds.compname)
  ))
