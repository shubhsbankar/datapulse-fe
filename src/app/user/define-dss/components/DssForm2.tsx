...
import { getTableColumnsAsync, createRdvCompDsAsync, testRdvCompDsAsync } from "@/store/userfeat/rdvcompdsThunks";
// import { getTableColumnsAsync } from "@/store/userfeat/rdvcompdhThunks";
import SortableMultiSelect from "@/components/ui/multiselect";

interface DssForm2Props {
  datasets: Dataset[];
  ...
  setQueryResult: (results: { headers: string[], rows: any[][], error: string } | null) => void;
}

export function DssForm2({
  datasets,
  tenantBkccs,
  selectedProject,
  setSelectedProject,
  ...
  const uniquieDLnames = useAppSelector(state => new Set(state.userfeat.rdvcompdl.filter(dl => dl.projectshortname == selectedProject && 
    dl.dpname == selectedDataProduct && dl.dsname == selectedDataset && dl.comptype == 'dl'
  ).map(dl => dl.compname)));
