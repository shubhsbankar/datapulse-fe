...
import { BrgTable } from './BrgTable';

interface SgTabsProps {
  onExport: (data: any[], filename: string) => void;
}

export function BrgTabs({
  onExport
}: SgTabsProps) {
  ...
  const brg = useAppSelector((state) => state.userfeat.dvcompbrg)?.filter(dh => isProjectAssignedAndActive(dh.projectshortname));
  console.log({brg});
  return (
