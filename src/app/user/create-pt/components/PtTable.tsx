...
export function PtTable({ pt }: PtTableProps) {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<PtColumns>>(
    new Set([...Object.values(PtColumns), PtColumns.actions])
  );
  ...
  const paginatedPt = filteredPt.slice(startIndex, endIndex);
  const totalPages = Math.ceil(filteredPt.length / ITEMS_PER_PAGE);

  const uniqueProjects = Array.from(new Set(pt.map(d => d.projectshortname)));

  const toggleColumn = (column: PtColumns) => {
    ...
    <div className="bg-white shadow rounded-lg p-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="relative flex-1 min-w-[200px] max-w-md">
          ...
          <input
            type="text"
            placeholder="Search PT components..."
            value={searchTerm}
        ...
        <div className="relative">
          ...
          {showColumnFilter && (
            <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-md shadow-lg z-10">
              {Object.values(PtColumns).map(column => (
                ...
                {Object.values(PtColumns).map(column => (
                  visibleColumns.has(column) && (
                    <td key={column} className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {column === 'createdate' ? formatDate(pt[column] || '') :
                       column === 'sqltext' ? 
                       (Array.isArray(pt[column]) ? pt[column].join(', ') : pt[column]) :
                       pt[column as keyof DvCompPt]}
                    </td>
                  )
                ))}
                {/* <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  <button
                    onClick={() => handleUpdateClick(pt.rdvid || 0)}
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="text-sm text-gray-700">
          ...
          {Math.min(currentPage * ITEMS_PER_PAGE, filteredPt.length)} of{' '}
          {filteredPt.length} results
        </div>
