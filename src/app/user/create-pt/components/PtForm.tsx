...
export function PtForm({
 ...
 const dsComponents = useAppSelector(state =>
  state.userfeat.rdvcompds.filter(ds => ds.assoccomptype === 'dh')
   ...
   const availableSatlNames = Array.from(new Set(dsComponents.filter(ds => ds.projectshortname == selectedProject && ds.assoccompname === dhName).map(ds => ds.satlname)));
  ...
  const handleValidate = async () => {
    ...
    try {
      const payload: DvCompPt = {
        ...
        compshortname: `${selectedProject}_${componentType}_${componentSubtype}_${componentName}_${dhName}_${version}`.toLowerCase(),
        satlnums: satlNums,
        comments,
        version,
        dhname : dhName,
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select
            id="dhName"
            value={dhName}
            onChange={(e) => setDhName(e.target.value as any)}
            className="mt-1 block w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500 sm:text-sm"
            required
          >
