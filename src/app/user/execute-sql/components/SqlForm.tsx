...
export function SqlForm({
  ...
  const uniqueProjects = Array.from(new Set(dtgspsn)).filter(a => isProjectActive(a));
  ...
  const availableDataProducts = dtgs.filter(d => d.projectshortname == selectedProject).map(dtg => dtg.dataproductshortname).filter((value, index, self) => self.indexOf(value) === index);
  console.log({dtgs, availableDataProducts});
  ...
  //   .filter((value, index, self) => self.indexOf(value) === index);
  const availableDatasets = dtgs.filter(d => d.projectshortname === selectedProject && d.dataproductshortname === selectedDataProduct).map(dtg => dtg.datasetshortname).filter((value, index, self) => self.indexOf(value) === index);

  // Get unique test coverage versions from dtgs
  const availableTestVersions = selectedDataset ? Array.from(
    new Set(dtgs.filter(d => 
      d.datasetshortname === selectedDataset && d.projectshortname === selectedProject && d.dataproductshortname === selectedDataProduct
    ).map(dtg => dtg.testcoverageversion))
  ).sort() : [];
