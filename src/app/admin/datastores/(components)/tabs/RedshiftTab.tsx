...
interface RedshiftTabProps {
  datastore: Datastore;
  onChange: (field: keyof Datastore, value: string) => void;
}

export function RedshiftTab({ datastore, onChange }: RedshiftTabProps) {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <div>
        ...
        >
          <option value="">Select Type</option>
          {DATASTORE_TYPES.redshift.map(type => (
      ...
      <div>
        <label htmlFor="aws_iam_role" className="block text-sm font-medium text-gray-700">
          AWS IAM Role
          <span className="text-xs text-gray-500 ml-1">(e.g., arn:aws:iam::0123456789012:role/myrsdbRole)</span>
        </label>
        <input
          type="text"
          id="aws_iam_role"
          name="aws_iam_role"
          value={datastore.aws_iam_role || ''}
          onChange={(e) => onChange('aws_iam_role', e.target.value)}
          className="mt-1 block w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500 sm:text-sm"
          placeholder="arn:aws:iam::0123456789012:role/myrsdbRole"
        />
      </div>
      <div>
        <label htmlFor="tempdir" className="block text-sm font-medium text-gray-700">
          Temp Directory
          <span className="text-xs text-gray-500 ml-1">(e.g., s3a://my-s3-bucket/temp/)</span>
        </label>
        <input
          type="text"
          id="tempdir"
          name="tempdir"
          value={datastore.tempdir || ''}
          onChange={(e) => onChange('tempdir', e.target.value)}
          className="mt-1 block w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500 sm:text-sm"
          placeholder="s3a://my-s3-bucket/temp/"
        />
      </div>
    </div>
  );
